 ...
 *
 * FIX 2026-08-22 (precisión RAG):
 *   - Filtro anti-ruido institucional: designaciones/ceses/caja chica NO se
 *     indexan por defecto (metadata.rag_value='ruido'). Override: --incluirRuido.
 *   - Campos objeto (ej. acuerdos_y_resoluciones_observancia_obligatoria del
 *     catálogo tribunal-fiscal) se normalizan a array antes de iterar.
 *   - metadata.materia se completa desde el filename si el doc no la trae.
 *
 * REINDEX SELECTIVO (NO ejecutar en esta sesión — DB prod). Comando canónico
 * para aplicar los fixes sobre lo ya indexado SIN reindex completo:
 *
 *   node tools/rag/index-todos.mjs --mode=hash --soloFaltantes --excluirRuido
 *
 * (`--excluirRuido` es el comportamiento default; el flag queda documentado
 * como explícito. `--incluirRuido` revierte la exclusión si se necesita auditar.)
 *
 * Requiere:
...
function parseArgs(argv) {
  const args = {
    limit: null, dryRun: false, only: null, soloFaltantes: false, mode: 'auto',
    incluirRuido: false, excluirRuido: false
  };
  for (const arg of argv) {
    ...
    // FIX 2026-08-22: alias --mode=hash / --mode=minimax (comando canónico documentado)
    if (arg.startsWith('--mode=')) {
      const m = arg.split('=')[1];
      args.mode = m === 'hash' ? 'hash' : m === 'minimax' ? 'minimax' : 'auto';
    }
    // FIX 2026-08-22 (ruido institucional): default EXCLUYE; --incluirRuido override.
    // --excluirRuido es no-op explícito (documentado en el comando canónico).
    if (arg === '--incluir-ruido' || arg === '--incluirRuido') args.incluirRuido = true;
    if (arg === '--excluir-ruido' || arg === '--excluirRuido') args.excluirRuido = true;
  }
...
/**
 * FIX 2026-08-22: normaliza un campo del catálogo a array antes de iterar.
 * Algunos catálogos (ej. resoluciones-tribunal-fiscal-2026.json →
 * `acuerdos_y_resoluciones_observancia_obligatoria`) modelan la colección
 * como OBJETO (mapa nota/url/listas) en vez de array; el indexer esperaba
 * array y saltaba el contenido completo. Aplica a cualquier campo similar:
 *   - array            → tal cual
 *   - objeto           → Object.values(), aplanando arrays anidados 1 nivel
 *                        (los strings sueltos se descartan luego por el
 *                        filtro `typeof d === 'object'`)
 *   - null/undefined/primitivo → []
 */
function normalizarAArray(x) {
...
function extractDocuments(catalog) {
  ...
  for (const key of CONFIG.arrayKeys) {
    // FIX 2026-08-22: normalizar objeto→array (tribunal-fiscal y similares)
    const rawDocs = normalizarAArray(catalog[key]);
    if (rawDocs.length === 0) continue;
    // Filtrar docs que no parezcan documentos (strings/URLs planas)
    const validDocs = rawDocs.filter((d) => d && typeof d === 'object' && !Array.isArray(d));
...
function resolveMateria(doc, sourceFile, catalogKey) {
  ...
  // FIX 2026-08-22 (materia gaps): si el doc no trae `materia` y el catálogo
  // no tiene mapa propio, inferir desde el filename (ej. casaciones-penales
  // → penal; normas-mtpe → laboral). Mejora filtros downstream {materia}.
  return materiaPorFilename(sourceFile) || 'general';
}

// Reglas filename→materia (orden = prioridad). Solo cubre fuentes RAG sin
// mapa de materia propio; los catálogos con doc.materia o catalogKey tienen
// precedencia en resolveMateria.
const FILENAME_MATERIA_RULES = [
  ['casaciones-penales', 'penal'],
  ['tipos-penales', 'penal'],
  ['delitos-economicos', 'penal_economico'],
  ['jurisprudencia-tc', 'constitucional'],
  ['sentencias-tc', 'constitucional'],
  ['tribunal-fiscal', 'tributario'],
  ['sunat', 'tributario'],
  ['sbs', 'bancario'],
  ['oefa', 'ambiental'],
  ['anpd', 'datos_personales'],
  ['indecopi', 'consumidor'],
  ['osce', 'contrataciones'],
  ['sunarp', 'registral'],
  ['mtpe', 'laboral'],
  ['minsa', 'sanitario'],
  ['onp', 'previsional'],
  ['minjusdh', 'administrativo'],
  ['elperuano', 'normativa_general'],
  ['plazos-procesales', 'procesal']
];

function materiaPorFilename(sourceFile) {
  const f = String(sourceFile || '').toLowerCase();
  for (const [needle, materia] of FILENAME_MATERIA_RULES) {
    if (f.includes(needle)) return materia;
  }
  return null;
}
...
// ==========================================
// FIX 2026-08-22: FILTRO ANTI-RUIDO INSTITUCIONAL
// ==========================================
//
// Los catálogos normas-mtpe/minsa/onp/cgr/elperuano (y boletines de otros
// entes) mezclan contenido SIN valor jurídico recuperable: designaciones de
// personal, ceses, encargos de despacho, resoluciones de caja chica,
// reconocimientos de servicios. Indexarlos contamina el retrieval con chunks
// irrelevantes para consultas legales reales.
//
// Heurística (2 condiciones, ambas requeridas):
//   1. El texto matchea patrones de acto administrativo de personal/caja.
//   2. NO contiene NINGÚN marcador de contenido normativo (artículo/norma/
//      ley/decreto/reglamento) → no es cita ni extracto de norma.
//
// Los chunks marcados reciben metadata.rag_value='ruido' y se EXCLUYEN del
// índice por defecto. Override: --incluirRuido (quedan indexados pero
// etiquetados, filtrables downstream con rag_value!='ruido').

const RUIDO_INSTITUCIONAL_RE =
  /designaci[oó]n|cese|encargar (despacho)|aceptar renuncia|caja chica|encargatura|reconocer (servicios|derechos)/i;

const VALOR_JURIDICO_RE = /art[ií]culo|norma|ley|decreto|reglamento/i;

/**
 * @param {object} doc - Documento del catálogo
 * @returns {boolean} true si el doc es ruido institucional sin valor RAG
 */
export function esRuidoInstitucional(doc) {
  if (!doc || typeof doc !== 'object') return false;
  const texto = buildCompositeText(doc);
  if (!texto) return false;
  return RUIDO_INSTITUCIONAL_RE.test(texto) && !VALOR_JURIDICO_RE.test(texto);
}
...
function buildBaseMetadata(doc, sourceFile, catalogKey) {
  return {
    ...
    // FIX 2026-08-22: etiqueta de valor RAG — 'ruido' = acto institucional sin
    // valor jurídico (excluido del índice salvo --incluirRuido); 'contenido' =
    // chunk con valor legal normal. Permite filtrado downstream explícito.
    rag_value: esRuidoInstitucional(doc) ? 'ruido' : 'contenido'
  };
}
...
async function main() {
  ...
  if (ARGS.dryRun) {
    ...
    for (const sourceFile of sources) {
      ...
      for (const doc of docs) {
        ...
        // FIX 2026-08-22: excluir ruido institucional también en dry-run
        if (!ARGS.incluirRuido && esRuidoInstitucional(doc)) {
          ruidoExcluido++;
          docCount++;
          continue;
        }
    ...
    console.log(`📊 DRY-RUN: ${docCount} documentos → ${chunkCount} chunks (${ruidoExcluido} docs ruido institucional excluidos)`);
  ...
  for (const sourceFile of fuentesAProcesar) {
    ...
    for (const doc of docs) {
      ...
      // FIX 2026-08-22: EXCLUIR ruido institucional por defecto. Los chunks
      // ya llevan metadata.rag_value='ruido' (buildBaseMetadata), así que con
      // --incluirRuido se indexan etiquetados y quedan filtrables downstream.
      if (!ARGS.incluirRuido && esRuidoInstitucional(doc)) {
        totalRuidoExcluido++;
        docCount++;
        continue;
  ...
  console.log(`   Docs ruido institucional EXCLUIDOS: ${totalRuidoExcluido}${ARGS.incluirRuido ? ' (--incluirRuido activo: indexados con rag_value=ruido)' : ''}`);
