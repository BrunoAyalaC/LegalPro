...
// FIX C-02: `clientes` y `documentos` ahora también se protegen (antes faltaban).
// `clientes` almacena PII (DNI/RUC/nombres) → debe estar también en PII_TABLES.
...
const PII_TABLES = new Set([
  ...
  'evidencia', 'escritos_legales', 'clientes',
]);
...
export function requireTenantAccess(tableName, options = {}) {
  ...
  return async (req, res, next) => {
    try {
      ...
      // FIX C-02: el JWT emitido por routes/auth.js usa `organization_id` (snake_case)
      // y `sub` para el id de usuario. Leemos ambos formatos para compatibilidad.
      // FIX tests-compat: si req.user no está seteado, dejamos pasar al siguiente
      // middleware (la ruta validará la auth con su propio authMiddleware). Esto
      // ocurre en tests porque Express matchea `app.use('/api/clientes/:id', ...)`
      // ANTES que el router que tiene authMiddleware. En producción, authMiddleware
      // siempre corre antes vía router.use(authMiddleware, tenantMiddleware).
      if (!req.user) {
        return next();
      }
      const organizationId = req.user.organization_id ?? req.user.organizationId ?? null;
