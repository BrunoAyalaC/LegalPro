/**
 * LPDP Art. 21 — Bloquea llamadas a MiniMax si el usuario no consintió
...
export function requireTransferenciaInternacional() {
  return async (req, res, next) => {
    try {
      ...
      // Fail-closed LPDP (P0): si no podemos verificar el consentimiento, BLOQUEAMOS
      console.error('[transferencia] Error al verificar consentimiento (fail-closed):', err.message);
      await logAudit('CONSENT_CHECK_FAILED', {
        severity: 'CRITICAL',
      ...
      return res.status(503).json({
        success: false,
        error: 'No se pudo verificar el consentimiento de transferencia internacional. Intente nuevamente en 30 segundos.',
