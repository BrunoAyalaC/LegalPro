...
async function obtenerPlanOrganizacion(orgId) {
  ...
  // FIX (2026-08-22): Promise.resolve() envuelve la llamada para que `.catch`
  // siempre exista aunque el módulo de cache devuelva un valor NO-Promise
  // (p. ej. mocks sync en tests: isAvailable: () => true). Sin el wrapper,
  // `(true).catch` lanza TypeError → fail-closed 503 espurio.
  const redisOk = await Promise.resolve(cache.isAvailable()).catch(() => false);
  ...
  if (redisOk) {
    ...
    // 2. Intentar desde memoria local si Redis no disponible
...
export function quotaMiddleware(tipo) {
  return async (req, res, next) => {
    ...
    try {
      ...
      console.error('[quota] Error en middleware de cuotas (fail-closed):', err);
      logger.error('quota_check_failed', { error: err.message, orgId, tipo });
      // Fail-closed LPDP + quota (P0): si no podemos verificar el límite, bloqueamos
      await logAudit('QUOTA_CHECK_FAILED', {
        severity: 'CRITICAL',
      ...
      return res.status(503).json({
        success: false,
        error: 'Servicio de cuotas temporalmente no disponible. Intente nuevamente en 30 segundos.',
