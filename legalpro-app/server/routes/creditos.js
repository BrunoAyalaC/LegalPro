...
// FIX C-03: importar tenantMiddleware desde tenantMiddleware.js (NO desde
// authMiddleware.js). La versión de authMiddleware.js solo setea req.organizationId;
// la de tenantMiddleware.js además envuelve next() en tenantContext.run(...),
// que es lo que permite a tenantQuery() ejecutar SET LOCAL y activar RLS.
...
router.get('/saldo', authMiddleware, tenantMiddleware, async (req, res, next) => {
  try {
    ...
    // FIX C-03: tenantQuery en lugar de db.query — activa RLS via SET LOCAL
    // (organizaciones no tiene policy RLS, pero es consistente con el contexto tenant)
...
router.get('/transacciones', authMiddleware, tenantMiddleware, async (req, res, next) => {
  try {
    const orgId = req.organizationId;
    // FIX C-03: tenantQuery — transacciones_creditos TIENE RLS
    // (policy p_transacciones_creditos_all: organization_id = fn_rls_current_org_id()).
    // Con db.query directo (sin contexto) la policy devolvería 0 filas o fallaría.
    const { rows } = await tenantQuery(
...
router.post('/comprar', authMiddleware, tenantMiddleware, async (req, res, next) => {
  try {
    ...
    // FIX C-03: tenantQuery en lugar de db.query (consistencia de contexto tenant)
    ...
    try {
      ...
      // FIX C-03: activar contexto RLS (SET LOCAL) dentro de la transacción manual.
      // tenantQuery() no soporta transacciones multi-statement, así que replicamos
      // aquí lo que hace db.js: set_config(...) con is_local=true. Es OBLIGATORIO
      // para que el INSERT INTO transacciones_creditos pase la policy
      // p_transacciones_creditos_all (WITH CHECK organization_id = fn_rls_current_org_id())
      // y para que el UPDATE de organizaciones no dependa del default connection.
      if (orgId) {
      ...
      await client.query("SELECT set_config('app.current_user_rol', $1, true)", [req.user?.rol || 'ABOGADO']);
