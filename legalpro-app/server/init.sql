...
-- -----------------------------------------------------------------------------
-- TABLA: clientes (CRM básico para estudios jurídicos)
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS clientes (
    ...
    estado_civil            TEXT        CHECK (estado_civil IN ('soltero', 'casado', 'divorciado', 'viudo', 'conviviente') OR estado_civil IS NULL),
    -- Persona jurídica
    razon_social            TEXT,
    ruc                     CHAR(11)    UNIQUE,
...
CREATE INDEX IF NOT EXISTS idx_clientes_nombre ON clientes USING gin(to_tsvector('spanish', COALESCE(nombre_completo, '') || ' ' || COALESCE(razon_social, '')));

COMMENT ON TABLE clientes IS 'CRM básico: clientes del estudio (demandantes, demandados, terceros). Aislado por organización.';
...
CREATE POLICY p_usuarios_insert ON usuarios
    FOR INSERT
    WITH CHECK (
        -- FIX C-02 (CRITICAL): antes era WITH CHECK (TRUE) → cualquier usuario
        -- autenticado podía insertar filas arbitrarias (p.ej. crear un ADMIN).
        -- Ahora el INSERT exige:
        --   1) Un rol válido del sistema (defensa en profundidad, alineado con
        --      el CHECK constraint de la tabla).
        --   2) Que la fila insertada sea el propio usuario autenticado
        --      (id = fn_rls_current_user_id()) O que NO haya contexto de sesión
        --      activo (fn_rls_current_user_id() IS NULL), que es el caso del
        --      registro público POST /api/auth/register (no pasa por
        --      tenantMiddleware ni establece app.current_user_id).
        rol IN ('OWNER', 'ADMIN', 'MEMBER', 'ABOGADO', 'FISCAL', 'JUEZ', 'CONTADOR', 'VIEWER')
        AND (
            id = fn_rls_current_user_id()
            OR fn_rls_current_user_id() IS NULL
        )
    );
...
-- -----------------------------------------------------------------------------
-- RLS: clientes — aislamiento multi-tenant
-- -----------------------------------------------------------------------------
ALTER TABLE clientes ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS p_clientes_all ON clientes;
CREATE POLICY p_clientes_all ON clientes
    FOR ALL
    USING (organization_id = fn_rls_current_org_id())
    WITH CHECK (organization_id = fn_rls_current_org_id());

COMMENT ON POLICY p_clientes_all ON clientes IS 'Clientes visibles solo para usuarios de la misma organización';

-- ══════════════════════════════════════════════════════════════════════════════
-- RLS: simulaciones — aislamiento multi-tenant
-- ══════════════════════════════════════════════════════════════════════════════
ALTER TABLE simulaciones ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS p_simulaciones_all ON simulaciones;
CREATE POLICY p_simulaciones_all ON simulaciones
    FOR ALL
    USING (organization_id = fn_rls_current_org_id())
    WITH CHECK (organization_id = fn_rls_current_org_id());

COMMENT ON POLICY p_simulaciones_all ON simulaciones IS 'Simulaciones visibles solo para usuarios de la misma organización';

-- ══════════════════════════════════════════════════════════════════════════════
-- RLS: mensajes_chat — aislamiento multi-tenant
-- ══════════════════════════════════════════════════════════════════════════════
ALTER TABLE mensajes_chat ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS p_mensajes_chat_all ON mensajes_chat;
CREATE POLICY p_mensajes_chat_all ON mensajes_chat
    FOR ALL
    USING (organization_id = fn_rls_current_org_id())
    WITH CHECK (organization_id = fn_rls_current_org_id());

COMMENT ON POLICY p_mensajes_chat_all ON mensajes_chat IS 'Mensajes de chat visibles solo para la propia organización';

-- ══════════════════════════════════════════════════════════════════════════════
-- RLS: notificaciones_sinoe — aislamiento multi-tenant
-- ══════════════════════════════════════════════════════════════════════════════
ALTER TABLE notificaciones_sinoe ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS p_notificaciones_sinoe_all ON notificaciones_sinoe;
CREATE POLICY p_notificaciones_sinoe_all ON notificaciones_sinoe
    FOR ALL
    USING (organization_id = fn_rls_current_org_id())
    WITH CHECK (organization_id = fn_rls_current_org_id());

COMMENT ON POLICY p_notificaciones_sinoe_all ON notificaciones_sinoe IS 'Notificaciones SINOE visibles solo para la propia organización';

-- ══════════════════════════════════════════════════════════════════════════════
-- RLS: evidencia_digital — aislamiento multi-tenant (con inmutabilidad)
-- ══════════════════════════════════════════════════════════════════════════════
...
COMMENT ON POLICY p_evidencia_digital_all ON evidencia_digital IS 'Evidencia digital visible solo para la propia organización. Mantiene inmutabilidad vía trigger separado.';

-- ══════════════════════════════════════════════════════════════════════════════
-- RLS: audit_log — aislamiento multi-tenant (insert-only por seguridad)
-- ══════════════════════════════════════════════════════════════════════════════
...
-- Audit log es INSERT-only por seguridad: no se permite UPDATE ni DELETE
DROP POLICY IF EXISTS p_audit_log_insert ON audit_log;
CREATE POLICY p_audit_log_insert ON audit_log
    FOR INSERT
    WITH CHECK (organization_id = fn_rls_current_org_id());

COMMENT ON POLICY p_audit_log_select ON audit_log IS 'Audit log visible solo para la propia organización (read-only)';
COMMENT ON POLICY p_audit_log_insert ON audit_log IS 'Audit log: solo INSERT permitido por la propia organización';

-- ══════════════════════════════════════════════════════════════════════════════
-- RLS: consumo_tokens_ia — aislamiento multi-tenant (billing)
-- ══════════════════════════════════════════════════════════════════════════════
ALTER TABLE consumo_tokens_ia ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS p_consumo_tokens_ia_all ON consumo_tokens_ia;
CREATE POLICY p_consumo_tokens_ia_all ON consumo_tokens_ia
    FOR ALL
    USING (organization_id = fn_rls_current_org_id())
    WITH CHECK (organization_id = fn_rls_current_org_id());

COMMENT ON POLICY p_consumo_tokens_ia_all ON consumo_tokens_ia IS 'Consumo de tokens IA visible solo para la propia organización (billing)';

-- ══════════════════════════════════════════════════════════════════════════════
-- [BUG#2 FIX 2026-06-29] RLS: eventos_simulacion — aislamiento multi-tenant
-- ══════════════════════════════════════════════════════════════════════════════
ALTER TABLE eventos_simulacion ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS p_eventos_simulacion_all ON eventos_simulacion;
CREATE POLICY p_eventos_simulacion_all ON eventos_simulacion
    FOR ALL
    USING (organization_id = fn_rls_current_org_id())
    WITH CHECK (organization_id = fn_rls_current_org_id());

COMMENT ON POLICY p_eventos_simulacion_all ON eventos_simulacion IS 'Eventos de simulacion visibles solo para la propia organizacion';

-- ══════════════════════════════════════════════════════════════════════════════
-- [BUG#2 FIX 2026-06-29] RLS: invitaciones_organizacion — aislamiento multi-tenant
-- ══════════════════════════════════════════════════════════════════════════════
ALTER TABLE invitaciones_organizacion ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS p_invitaciones_organizacion_all ON invitaciones_organizacion;
CREATE POLICY p_invitaciones_organizacion_all ON invitaciones_organizacion
    FOR ALL
    USING (organization_id = fn_rls_current_org_id())
    WITH CHECK (organization_id = fn_rls_current_org_id());

COMMENT ON POLICY p_invitaciones_organizacion_all ON invitaciones_organizacion IS 'Invitaciones visibles solo para la propia organizacion';

-- ══════════════════════════════════════════════════════════════════════════════
-- [BUG#2 FIX 2026-06-29] RLS: transacciones_creditos — aislamiento multi-tenant (billing)
-- ══════════════════════════════════════════════════════════════════════════════
ALTER TABLE transacciones_creditos ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS p_transacciones_creditos_all ON transacciones_creditos;
CREATE POLICY p_transacciones_creditos_all ON transacciones_creditos
    FOR ALL
    USING (organization_id = fn_rls_current_org_id())
    WITH CHECK (organization_id = fn_rls_current_org_id());

COMMENT ON POLICY p_transacciones_creditos_all ON transacciones_creditos IS 'Transacciones de creditos visibles solo para la propia organizacion (billing)';

-- ══════════════════════════════════════════════════════════════════════════════
-- RESUMEN: 13 tablas con RLS habilitado (10 existentes + 3 nuevas)
-- ══════════════════════════════════════════════════════════════════════════════
...
-- eventos_simulacion, invitaciones_organizacion, transacciones_creditos
-- ══════════════════════════════════════════════════════════════════════════════