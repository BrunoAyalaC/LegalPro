...
| 7   | **IALegal**       | `@IALegal`       | MiniMax SDK `minimaxClient.js`, Function Calling       |
...
| 3   | **generar-escrito-legal** | Escribir demandas, apelaciones, resoluciones con MiniMax |
| 4   | **configurar-minimax**    | Setup/debug del SDK `minimaxClient.js`, function calling   |
| 5   | **deploy-backend**        | Desplegar a Railway (Node o .NET)                       |
| 6   | **migrar-base-datos**     | Crear tablas, RLS policies en Supabase PostgreSQL       |
| 7   | **simulacion-juicio**     | Simulador interactivo de juicios con MiniMax AI          |
...
| `legal-prompts.instructions.md`   | `**/services/*ai*.{js,ts,cs}` | SDK `minimaxClient.js`, prompts legales |
...
¿Integrar MiniMax IA?       → @IALegal (skill: configurar-minimax)
...
## MiniMax Function Calling (Búsqueda en Tiempo Real)
...
| `redactar_escrito`      | Genera documentos        | MiniMax generativo                      |
...
| `generar_estrategia`    | Plan de acción legal     | MiniMax + contexto del caso             |
| `consultar_norma`       | Artículos legales        | Supabase: `base_legal_vectorial`       |

**Flujo**: MiniMax decide función → Backend Railway ejecuta → Query Supabase → Resultado vuelve a MiniMax → Respuesta contextualizada al usuario
