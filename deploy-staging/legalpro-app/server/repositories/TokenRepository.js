...
export class TokenRepository extends BaseRepository {
  /**
   * Calcula el costo estimado en USD de una llamada de IA basado en los precios de Gemini.
   */
  calcularCosto(modelo, promptTokens, completionTokens) {
    ...
    const isLite = modelLower.includes('lite');
    
    // Tarifas por token de Gemini (en USD por token individual)
    // Gemini 1.5/3.1/3.5 Pro: Input $1.25/M, Output $5.00/M
    // Gemini 1.5/3.1 Flash-Lite: Input $0.075/M, Output $0.30/M
    // Gemini 1.5/3.5 Flash: Input $0.075/M, Output $0.30/M
  ...
  async verificarCreditos(orgId) {
    ...
    return rows[0]?.creditos_disponibles ?? 0;
  }
    ...
    return this.transaction(async (client) => {
      ...
      const { rows: transRows } = await client.query(
        `INSERT INTO transacciones_creditos (
         ...
         VALUES ($1, $2, $3, $4, 'DEBITO', $5, 0, 'culqi', 'aprobado', $5, 'consumo_ia')
         RETURNING *`,
         [orgId, usuarioId, expedienteId || null, cantidad, motivo]
      );
