...
// Todos los endpoints requieren auth + tenant + consentimiento transferencia (LPDP Art. 21)
router.use(authMiddleware, tenantMiddleware, requireTransferenciaInternacional());
...
router.post('/chat', async (req, res, next) => {
  try {
    ...
    const cacheKey = hashKey('chat', mensajeSanitizado, MODEL, contextoExpediente);
...
router.post('/consulta', async (req, res, next) => {
  try {
    ...
    const cacheKey = hashKey('consulta', promptSanitizado, tipo, MODEL);
    const cached = await get(cacheKey);
    if (cached) {
