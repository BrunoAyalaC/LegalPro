...
export default function PanelCreditos() {
  ...
  // SEO Dinámico
  useEffect(() => {
    document.title = 'Mis Créditos y Consumos | LegalPro';
    ...
    metaDesc.setAttribute(
      'content',
      'Administra tus créditos y gemas de LegalPro. Recarga tu saldo, revisa el historial de transacciones y optimiza el consumo de la IA.'
    );
    ...
    metaKey.setAttribute(
      'content',
      'créditos legalpro, gemas, pasarela de pago, recarga de créditos, LexIA'
    );
  }, []);
  ...
  const cargarSaldo = useCallback(async () => {
    try {
      const res = await nodeClient.get('/api/organizaciones/me');
      const org = res.data?.data ?? res.data;
      if (org?.creditosDisponibles != null) setCreditos(org.creditosDisponibles);
    } catch (err) {
      console.error('[PanelCreditos] Error al cargar saldo:', err);
  ...
  const cargarPlanes = useCallback(async () => {
    try {
      const res = await nodeClient.get('/api/creditos/planes');
      const data = res.data?.planes ?? res.data?.data ?? [];
      setPlanes(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error('[PanelCreditos] Error al cargar planes:', err);
  ...
  const cargarTransacciones = useCallback(async () => {
    try {
      const res = await nodeClient.get('/api/creditos/transacciones');
      const data = res.data?.transacciones ?? res.data?.data ?? [];
      setTransacciones(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error('[PanelCreditos] Error al cargar transacciones:', err);
