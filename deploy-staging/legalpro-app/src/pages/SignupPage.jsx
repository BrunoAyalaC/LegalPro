...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(msg);
      console.error('[SignupPage] Error al registrar:', e);
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <fieldset className="space-y-4 mt-6">
          ...
          <label className="flex items-start gap-3 p-3 bg-slate-900 rounded border border-amber-700">
            <input
              ...
              className="mt-1"
              aria-label="Acepto transferencia internacional a Google Gemini"
            />
            <div>
              <span className="font-medium">Acepto la transferencia internacional a Google Gemini (EE.UU.)</span>
              <button
                type="button"
              ...
              {showDetalles.transferencia_internacional && (
                <div className="text-xs text-slate-300 mt-2 p-2 bg-slate-800 rounded">
                  <p><strong>Destino:</strong> Google LLC (Estados Unidos)</p>
                  ...
                  <p><strong>Protecciones:</strong> DPA con Google, encriptacion en transito, decision adequacy EU</p>
                </div>
              )}
