...
export default function RedactorEscritos() {
  ...
  // Cargar estado de revisión desde localStorage al generarse un nuevo documento
  useEffect(() => {
    if (resultado) {
      const saved = localStorage.getItem('legalpro_review_' + btoa(resultado.slice(0, 100)));
      if (saved) {
        try {
          const { status, comment, revisor } = JSON.parse(saved);
          setReviewStatus(status);
          setReviewComment(comment || '');
          setRevisorNombre(revisor || '');
        } catch {}
      } else {
  ...
  // Guardar estado de revisión en localStorage
  function saveReview(status, comment = '') {
    setReviewStatus(status);
    setReviewComment(comment);
    if (resultado) {
      const key = 'legalpro_review_' + btoa(resultado.slice(0, 100));
