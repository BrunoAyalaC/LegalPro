...
const DB_URL = process.env.DATABASE_PUBLIC_URL
  || 'postgresql://postgres:REMOVED_SECRET@metro.proxy.rlwy.net:42060/railway';

const candidatos = [
  resolve(__dir, 'server/init.sql'),
  resolve(__dir, '../docker/init.sql'),
];
const sqlPath = candidatos.find(p => existsSync(p));
if (!sqlPath) {
  console.error('No se encontró init.sql en ningún candidato:', candidatos);
  process.exit(1);
}
