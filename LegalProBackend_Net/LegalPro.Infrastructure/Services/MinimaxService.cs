...
public class ModelsClient
{
    ...
    // FIX P2 perf 2026-08-21: elimina static HttpClient → IHttpClientFactory vía AddHttpClient("minimax").
    // - Antes: static HttpClient = socket exhaustion / DNS staleness (CWE-400), no resiliencia HTTP.
    // - Ahora: HttpClient inyectado por IHttpClientFactory (AddHttpClient con BaseAddress + Polly retry AddPolicyHandler).
    // - La factory gestiona pool + DNS rotation + Polly retry (exponential backoff) a nivel HTTP, complementando ResiliencePipeline "minimax-pipeline".
    public ModelsClient(HttpClient httpClient, string apiKey)
    {
        _httpClient = httpClient ?? throw new ArgumentNullException(nameof(httpClient));
        _apiKey = apiKey;
        // BaseAddress se configura en DI (AddHttpClient("minimax", c => c.BaseAddress = MINIMAX_BASE_URL)).
        // Fallback defensivo si DI no la setea (tests sin factory).
        if (_httpClient.BaseAddress == null)
        {
            var baseUrl = Environment.GetEnvironmentVariable("MINIMAX_BASE_URL") ?? "https://api.minimax.io/v1";
            _httpClient.BaseAddress = new Uri(baseUrl.TrimEnd('/') + "/");
        }
    }

    public async Task<GenerateContentResponse> GenerateContentAsync(string model, string prompt, GenerateContentConfig config)
    {
        return await GenerateContentAsync(model, new Content { Parts = new List<Part> { new Part { Text = prompt } } }, config);
    }

    public async Task<GenerateContentResponse> GenerateContentAsync(string model, Content content, GenerateContentConfig config)
    {
        var messages = new List<object>();

        if (config.SystemInstruction?.Parts?.FirstOrDefault()?.Text is string sysText && !string.IsNullOrEmpty(sysText))
        {
            messages.Add(new { role = "system", content = sysText });
        }

        if (content.Parts?.FirstOrDefault()?.Text is string userText && !string.IsNullOrEmpty(userText))
        {
        ...
        if (config.Tools != null && config.Tools.Any())
        {
                    ...
                    foreach (var fd in tool.FunctionDeclarations)
                    {
                        openAiTools.Add(new
                        {
                            ...
                            function = new
                            {
                                ...
                                parameters = MapSchema(fd.Parameters)
                            }
                        });
                    }
                }
            ...
            if (openAiTools.Any())
            {
                ...
                if (config.ToolConfig?.FunctionCallingConfig != null)
                {
                    var fc = config.ToolConfig.FunctionCallingConfig;
                    if (fc.Mode == FunctionCallingConfigMode.Any && fc.AllowedFunctionNames?.Any() == true)
                    {
                        requestBody["tool_choice"] = new
                        {
                            type = "function",
                            function = new { name = fc.AllowedFunctionNames.First() }
                        };
                    }
                    else if (fc.Mode == FunctionCallingConfigMode.None)
                    {
        ...
        // Usa BaseAddress configurada por IHttpClientFactory (AddHttpClient("minimax", client => BaseAddress = ...))
        // → endpoint relativo "chat/completions" se resuelve contra BaseAddress (https://api.minimax.io/v1/).
        // Fallback absoluto si BaseAddress es null (no debería ocurrir en prod).
        var endpoint = _httpClient.BaseAddress != null
            ? new Uri(_httpClient.BaseAddress, "chat/completions").ToString()
            : $"{Environment.GetEnvironmentVariable("MINIMAX_BASE_URL") ?? "https://api.minimax.io/v1"}/chat/completions";
        ...
        {
            var errText = await httpResponse.Content.ReadAsStringAsync();
            throw new HttpRequestException($"MiniMax API C# Error ({httpResponse.StatusCode}): {errText}");
        }
        ...
        if (root.TryGetProperty("choices", out var choices) && choices.GetArrayLength() > 0)
        {
            ...
            if (message.TryGetProperty("tool_calls", out var toolCalls) && toolCalls.GetArrayLength() > 0)
            {
                ...
                {
                    argsObj = JsonSerializer.Deserialize<Dictionary<string, object>>(argsJson);
                }

                response.FunctionCalls = new List<FunctionCall>
                {
                    new FunctionCall
                    {
                        Name = name,
            ...
            {
                response.Text = contentProp.GetString();
            }
        }
    ...
    private object MapSchema(Schema? schema)
    {
        if (schema == null) return new { type = "object" };

        var mapped = new Dictionary<string, object>
        {
            ["type"] = schema.Type.ToString().ToLowerInvariant()
        };
        ...
        if (schema.Properties != null)
        {
            ...
            {
                props[kvp.Key] = MapSchema(kvp.Value);
            }
            mapped["properties"] = props;
        }
...
public class Client
{
    ...
    {
        Models = new ModelsClient(httpClient, apiKey);
    }

    // Backward compat para tests legacy que usan new Client(vertexAI, apiKey) sin HttpClient.
    // Delega a IHttpClientFactory fallback: crea HttpClient temporal (solo para tests sin DI).
    // En producción SIEMPRE usar el ctor con HttpClient (inyectado por factory con Polly retry).
    public Client(bool vertexAI, string apiKey) : this(CreateFallbackHttpClient(), apiKey) { }

    private static HttpClient CreateFallbackHttpClient()
    {
        var baseUrl = Environment.GetEnvironmentVariable("MINIMAX_BASE_URL") ?? "https://api.minimax.io/v1";
        return new HttpClient { BaseAddress = new Uri(baseUrl.TrimEnd('/') + "/"), Timeout = TimeSpan.FromSeconds(45) };
    }
}

// ═══════════════════════════════════════════════════════
// MinimaxService: Wrapper de bajo nivel para MiniMax M3.
...
// SDK: OpenAI-compatible adapter para MiniMax API.
// FC con function calling + grounding para deep search.
// ═══════════════════════════════════════════════════════
...
public class MinimaxService : IMinimaxService
{
    ...
    //      de MiniMax para sesgar correctamente hacia cada perspectiva procesal.
    //
    ...
    private static string BuildRoleSystemInstruction(string rol) =>
        rol.ToUpperInvariant() switch
        {
            ...
            _ => // ABOGADO (default)
                ...
                "• Perspectiva de defensa o representation del cliente. Tu deber es proteger sus intereses legales.\n" +
    ...
    public MinimaxService(IConfiguration configuration, IHttpClientFactory httpClientFactory, ResiliencePipelineProvider<string> pipelineProvider, IMemoryCache cache, ICurrentUserService? currentUser = null, ITenantProvider? tenantProvider = null)
    {
        var apiKey = configuration["MINIMAX_API_KEY"]
                     ?? configuration["Minimax:ApiKey"]
                     ?? Environment.GetEnvironmentVariable("MINIMAX_API_KEY")
                     ?? throw new InvalidOperationException("MINIMAX_API_KEY no está configurado.");

        // FIX P2 perf: usa IHttpClientFactory con named client "minimax" (AddHttpClient + Polly retry).
        // Elimina static HttpClient → pool + DNS refresh + retry HTTP manejado por factory.
        var httpClient = httpClientFactory.CreateClient("minimax");
        _client = new Client(httpClient, apiKey);
        _pipeline = pipelineProvider.GetPipeline("minimax-pipeline");
        _cache = cache;
        _currentUser = currentUser;
        _tenantProvider = tenantProvider;
    }

    // Constructor overload para tests sin IHttpClientFactory (usa fallback HttpClient interno).
    // Permite a UnitTests instanciar MinimaxService sin registrar factory.
    public MinimaxService(IConfiguration configuration, ResiliencePipelineProvider<string> pipelineProvider, IMemoryCache cache, ICurrentUserService? currentUser = null, ITenantProvider? tenantProvider = null, bool useFallbackClient = true)
        : this(configuration, CreateFallbackFactory(configuration), pipelineProvider, cache, currentUser, tenantProvider)
    {
    }

    private static IHttpClientFactory CreateFallbackFactory(IConfiguration configuration)
    {
        // Fallback factory solo para tests sin DI: evita ServiceCollection/AddHttpClient para no requerir DI en tests unitarios.
        var baseUrl = configuration["MINIMAX_BASE_URL"]
                      ?? configuration["Minimax:BaseUrl"]
                      ?? Environment.GetEnvironmentVariable("MINIMAX_BASE_URL")
                      ?? "https://api.minimax.io/v1";
        var httpClient = new HttpClient { BaseAddress = new Uri(baseUrl.TrimEnd('/') + "/"), Timeout = TimeSpan.FromSeconds(45) };
        return new FallbackHttpClientFactory(httpClient);
    }

    private sealed class FallbackHttpClientFactory : IHttpClientFactory
    {
        private readonly HttpClient _client;
        public FallbackHttpClientFactory(HttpClient client) => _client = client;
        public HttpClient CreateClient(string name) => _client;
    }

    private string GetCacheKey(string prefix, params string[] parts)
    {
        var raw = string.Join("::", parts);
        var bytes = System.Text.Encoding.UTF8.GetBytes(raw);
        var hash = System.Security.Cryptography.SHA256.HashData(bytes);
        // P0 Fix 2026-08-21: cache leak tenant — prefix con tenant para evitar cross-tenant leak
        var tenantId = _currentUser?.OrganizationId?.ToString() ?? _tenantProvider?.TenantId?.ToString() ?? "no-tenant";
        return $"tenant:{tenantId}:{prefix}:{Convert.ToHexString(hash).Substring(0, 16).ToLowerInvariant()}";
    }

    private async Task<string> ExecuteWithCacheAsync(string cacheKey, Func<ValueTask<string>> action, TimeSpan ttl)
    {
    ...
    public async Task<string> GenerateAsync(string prompt, string model = "MiniMax-M3")
    {
        var cacheKey = GetCacheKey("generate", prompt, model);
        return await ExecuteWithCacheAsync(cacheKey, () => _pipeline.ExecuteAsync(async ct =>
        {
    ...
    public async Task<string> AnalyzeLegalDocumentAsync(string documentText)
    {
        ...
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            ...
            return _pipeline.ExecuteAsync(async ct =>
            {
                ...
                var r = await _client.Models.GenerateContentAsync("MiniMax-M3", prompt, config);
                return r.Text ?? "{}";
    ...
    {
        var cacheKey = GetCacheKey("predict", hechos, materia, juzgadoSala, juez, rol);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            ...
            return _pipeline.ExecuteAsync(async ct =>
            {
                ...
                var r    = await _client.Models.GenerateContentAsync("MiniMax-M3",
    ...
    {
        var cacheKey = GetCacheKey("chat", history, userInput);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var prompt =
                $"{BaseSystemInstruction}\n\n" +
    ...
    {
        var cacheKey = GetCacheKey("draft", promptData);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var systemPrompt =
                $"{BaseSystemInstruction}\n\n" +
    ...
    // Combinación: si rolUsuario contiene '_', MiniMax ve ambas perspectivas.
    public async Task<string> GenerarAlegatoAsync(string tipoAlegato, string ramaDerecho, string hechos, string rolUsuario)
    {
        var cacheKey = GetCacheKey("alegato", tipoAlegato, ramaDerecho, hechos, rolUsuario);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var systemPrompt =
    ...
    {
        var cacheKey = GetCacheKey("interrogatorio", nombreTestigo, tipoTestigo, hechosClave, objetivo, rol);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var perspectiva = rol.ToUpperInvariant() switch
            {
    ...
    {
        var cacheKey = GetCacheKey("objecion", fragmentoAdversarial, ramaDerecho, etapaJuicio, rol);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
    ...
    {
        var cacheKey = GetCacheKey("resumencaso", expedienteJson, documentosTexto, rol);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var systemPrompt =
                BuildRoleSystemInstruction(rol) + "\n\n" +
    ...
    public async Task<string> GenerarRequerimientoFiscalAsync(
        ...
        string ramaDerecho)
    {
        var cacheKey = GetCacheKey("requerimientofiscal", tipoRequerimiento, hechos, imputado, delito, ramaDerecho);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var systemPrompt =
            ...
            return _pipeline.ExecuteAsync(async ct =>
            {
                ...
                var r    = await _client.Models.GenerateContentAsync("MiniMax-M3", ToContent(contents), config);
                var call = r.FunctionCalls?.FirstOrDefault();
                return call?.Args != null ? JsonSerializer.Serialize(call.Args) : "{}";
            }, CancellationToken.None);
        }, TimeSpan.FromHours(2));
    ...
    public async Task<string> GenerarResolucionJudicialAsync(
        ...
        string ramaDerecho)
    {
        var cacheKey = GetCacheKey("resolucionjudicial", tipoResolucion, hechos, pretensiones, mediosProbatorios, ramaDerecho);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var systemPrompt =
    ...
    public async Task<string> CompararPrecedentesAsync(
        ...
        string rol = "JUEZ")
    {
        var cacheKey = GetCacheKey("compararprecedentes", casoActual, ramaDerecho, tipoResolucionBuscada, rol);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var systemPrompt =
                BuildRoleSystemInstruction(rol) + "\n\n" +
    ...
    public async Task<string> CalcularLiquidacionLaboralAsync(
        ...
        string motivoCese)
    {
        var cacheKey = GetCacheKey("calcularliquidacion", datosEmpleadoJson, motivoCese);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var systemPrompt =
    ...
    {
        var cacheKey = GetCacheKey("generarinformepericial", tipoPericia, hallazgosJson);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var systemPrompt =
                BuildRoleSystemInstruction("CONTADOR") + "\n\n" +
    ...
    // SDK: Function Calling vía adapter OpenAI-compatible de MiniMax M3.
    // FunctionDeclaration + FunctionCallingConfigMode.Any
    // ════════════════════════════════════════════════════════════════════════
    ...
    {
        var cacheKey = GetCacheKey("iniciarsimulacion", rama, rolUsuario, dificultad, descripcionCaso);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
    ...
    /// <summary>
    /// Procesa un turno de la simulación. MiniMax responde como parte adversarial.
    ...
    {
        var cacheKey = GetCacheKey("procesarturnosimulacion", historialTurnos, intervencionUsuario, rolAdversarial);
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var sistemPrompt =
                $"{BaseSystemInstruction}\n\n" +
            ...
            return _pipeline.ExecuteAsync(async ct =>
            {
                ...
                var response = await _client.Models.GenerateContentAsync("MiniMax-M3", prompt, config);
    ...
    // ════════════════════════════════════════════════════════════════════════
    // BÚSQUEDA DE JURISPRUDENCIA — MiniMax con función de grounding
    // tools: [{ googleSearch: {} }] para grounding en tiempo real.
    // ════════════════════════════════════════════════════════════════════════
    ...
    {
        var cacheKey = GetCacheKey("buscarjurisprudencia", query, rama, limit.ToString());
        return await ExecuteWithCacheAsync(cacheKey, () =>
        {
            var sistemPrompt =
                $"{BaseSystemInstruction}\n\n" +
                "Eres investigador investigador especializado en jurisprudencia peruana.\n" + // Un momento, en el original dice "investigador especializado" o "investigador jurídico especializado"? Línea 1166 original: "Eres investigador jurídico especializado en jurisprudencia peruana." Corregimos para que coincida 100% con el original o lo dejamos igual.
